-- =============================================================================
-- CRM OS · ACTUALIZACIÓN: migraciones 0017 en adelante (para un proyecto que YA tiene 0001 a 0016)
...
do $$ begin
  ...
  if to_regclass('public.tasks') is null and 17 > 10 then
  ...
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'channels' and column_name = 'connection_status') then
    raise exception 'FALTA la migración 0016 (Conexiones de WhatsApp). Instala primero setup-desde-0016.sql o avísame.';
  end if;
  if to_regclass('public.oauth_sessions') is not null then
    raise exception 'Este proyecto YA tiene instalada la migración 0017 (existe la tabla oauth_sessions). No ejecutes este archivo: avísame.';
  end if;
end $$;
