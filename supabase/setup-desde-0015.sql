-- =============================================================================
-- CRM OS · ACTUALIZACIÓN: migraciones 0015 en adelante (para un proyecto que YA tiene 0001 a 0014)
...
do $$ begin
  ...
  if to_regclass('public.tasks') is null and 15 > 10 then
  ...
  if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'opportunities' and column_name = 'number') then
    raise exception 'Este proyecto YA tiene instalada la migración 0015 (existe la columna number en opportunities). No ejecutes este archivo: avísame.';
  end if;
end $$;
